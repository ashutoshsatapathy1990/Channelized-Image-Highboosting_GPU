...
int main()
{
	...
	cl_uint num_devices;
	clStatus = clGetDeviceIDs(platforms[2], CL_DEVICE_TYPE_ALL, 0, NULL, &num_devices);
	device_list = (cl_device_id*)malloc(sizeof(cl_device_id) * num_devices);
	clStatus = clGetDeviceIDs(platforms[2], CL_DEVICE_TYPE_ALL, num_devices, device_list, NULL);
	...
	size_t global = Image.rows * Image.cols, local = 1024;
	...
	//Execute the OpenCL Kernels for Boosting of Each Channels Independetly.
	Exec_Kernel(command_queue_highboost, kernel_filtration, Image.rows*Image.cols, 1024, clStatus);
